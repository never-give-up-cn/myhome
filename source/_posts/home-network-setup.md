...
```
1. 确认 H3C 桥接接口状态：display interface Bridge-Aggregation 1
2. 确认桥接对是否正常：display bridge-aggregation
3. 检查安全策略是否误拦截：
   display security-policy ip
   display security-policy statistics
4. 确认软路由 PPPoE 拨号是否正常（桥接不受影响）
5. 跳过防火墙测试：临时将软路由 WAN 直连光猫，排查是否为防火墙故障
```
...
1. 从内网 ping 10.0.2.252：
   - 不通 → 依次 ping 192.168.1.1 → 192.168.100.1 → 192.168.100.254
   - 在哪一步断了，问题就出在哪一步
2. 确认软路由路由表：
   route -n 应包含 10.0.2.0/24 → 192.168.100.254
3. 确认 H3C 安全策略 trust-to-dmz 已启用
4. 确认 DMZ 服务器的网关指向 10.0.2.1（H3C GE1/0/3）
5. 从 H3C 直接 ping 目标 DMZ 设备确认存活
```
