...
### 新增监控 VLAN（10.0.3.0/24）

改造后将监控系统（NVR + 摄像头）从内网 `192.168.1.0/24` 迁移到防火墙独立 VLAN，实现物理/逻辑隔离。

#### 迁移前后对比

| 设备 | 改造前 IP | 改造后 IP |
|------|----------|----------|
| NVR 录像机 | 192.168.1.6 | **10.0.3.6** |
| 摄像头 1 | 192.168.1.8 | **10.0.3.8** |
| VLAN 网关 | — | **10.0.3.1（H3C GE1/0/4）** |
| 所属区域 | 路由器下（内网） | **H3C 防火墙独立 VLAN** |

#### 监控 VLAN 出网配置

监控设备需要连接外网（如远程查看、固件升级），通过 H3C 防火墙 NAT 出网：
...
| 设备 | 目标网段 | 下一跳 | 说明 |
|------|---------|--------|------|
| 软路由 192.168.100.1 | 10.0.2.0/24 + 10.0.3.0/24 | 192.168.100.254 | 去 DMZ + 监控走防火墙 |
| 路由器 192.168.1.1 | 10.0.2.0/24 + 10.0.3.0/24 | 192.168.100.1 | 去服务器区走软路由 |
| H3C 防火墙 | 192.168.1.0/24 | 192.168.100.1 | 内网访问走软路由 |

**软路由补充路由：**
```bash
route add -net 10.0.2.0/24 gw 192.168.100.254
route add -net 10.0.3.0/24 gw 192.168.100.254  # 新增监控段
```
...
| **软路由** 192.168.100.1 | **10.0.3.0/24** | 192.168.100.254 | LAN | 去监控 VLAN 走防火墙 |
| **路由器** 192.168.1.1 | 10.0.2.0/24 | 192.168.100.1 | WAN | 去 DMZ 走软路由 |
| **路由器** 192.168.1.1 | **10.0.3.0/24** | 192.168.100.1 | WAN | 去监控 VLAN 走软路由 |
